ActiveAdmin.register_page "Orders Not Invoiced" do
  menu false

  action_item :schedule_report do
    link_to "Schedule", admin_orders_not_invoiced_schedule_report_path, remote: true, class: 'scheduleReportBtn'
  end
  ...
  content do
    ...
    orders = ReportsDbBase.connection.exec_query("SELECT * FROM ORDER_NOT_INVOICED_VIEW WHERE DATE(delivery_at) BETWEEN '#{start_date}' AND '#{end_date}';")
    # orders = orders.group_by{ |key, value| key["created_at"].to_date }
    render partial: 'admin/reports/orders_not_invoiced', locals: { orders: orders, start_date: start_date, end_date: end_date }
  end
  ...
  page_action :csv, method: :get do
    ...
    send_data csv.encode('UTF-8'), type: 'text/csv; charset=utf-8; header=present', disposition: "attachment; filename=orders_not_invoiced_report.csv"
  end

  page_action :schedule_report, method: :get do
    @report = Report.find_by_report_type(Report.report_types[:orders_not_invoiced])
  ...
  page_action :scheduled_report, method: :post do
    ...
    if @report.present?
      ...
      @report = Report.new(name: params[:name], scheduled_period: Report.scheduled_periods[params["scheduled_period"]], scheduled_time: params[:scheduled_time].to_i, user_ids: params[:user_ids], report_type: :orders_not_invoiced, enable_error_logging: params[:enable_error_logging])
      if @report.save
        @success = true
        CronUpdaterJob.perform_later()
      else
