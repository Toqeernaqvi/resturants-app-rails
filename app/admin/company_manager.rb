ActiveAdmin.register CompanyManager, as: 'Company Manager' do
  menu false
  config.batch_actions = false
  actions :index, :create, :new, :edit, :update

  permit_params do
    if params[:company_manager] && !params[:company_manager][:password].blank?
    ...
    f.inputs do
      f.input :office_admin_id, as: :nested_select,
        ...
        level_2: {
          ...
          url: company_admins_admin_company_managers_path
  ...
  controller do
    ...
    def create
      params[:company_manager][:company_id] = params[:company_manager][:comp_id]
      params[:company_manager][:user_type] = "company_manager"
      super
    end

    def scoped_collection
      CompanyManager.unscoped
    end
  end
end
