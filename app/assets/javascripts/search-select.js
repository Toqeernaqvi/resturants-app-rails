$(function() {
  ...
  function setupSearchSelect(container) {
    $('.search-select-input, .search-select-filter-input, ajax-filter-input', container).each(function(i, el) {
      ...
      if ($("form.cuisineslist").length > 0) {
        var menu_type = $("select#cuisineslist_menu_type").val();
        url = url+"?menu_type="+menu_type;
      }
      else if ($("form.sequence").length > 0) {
        var menu_type = $("select#sequence_menu_type").val();
        url = url+"?menu_type="+menu_type;
      }
