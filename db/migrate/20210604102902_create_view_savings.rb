class CreateViewSavings < ActiveRecord::Migration[5.1]
  def self.up
    execute "CREATE OR REPLACE VIEW view_savings AS 
      ...
      ((companies.user_meal_budget * quantity) - total_price) AS savings, (orders.created_at AT TIME ZONE 'PST') AS created_at, (orders.updated_at AT TIME ZONE 'PST') AS updated_at
      FROM orders
  ...
  def self.down
    execute "DROP VIEW IF EXISTS view_savings"
  end
end
